    ...
    def __init__(self, mode=m.eVideoMode.MJPEGMode, exposure=50, delay_strobe=False):
        ...
        for i in range(len(self.camera_array)):
            ...
            if self.delay_strobe:
                self.camera_array[i].SetShutterDelay(int(self.exposure * 1.1 * i)) # Keep the cameras from firing into eachother?
                self.camera_array[i].SetStrobeOffset(int(self.exposure * 1.1 * i)) # Keep the cameras from firing into eachother?
